...
import {
  listPublishedWikiCollectionPagesByUniverseId,
  listPublishedWikiCollectionPagesByWikiSlug,
  type WikiCollectionListEntry
...
export type WikiRelatedData = {
  ...
  catalogPages: WikiCollectionListEntry[];
...
async function fetchWikiPage(slug: string): Promise<WikiPageContent | null> {
  ...
  return (data as WikiPageContent) ?? null;
}
...
export async function getWikiPageBySlug(slug: string): Promise<WikiPageContent | null> {
  ...
  const cached = publicContentCache(async () => fetchWikiPage(normalized), [`wiki-page-v4:${normalized}`], {
...
export async function getWikiByUniverseId(
  ...
  const cached = publicContentCache(
    ...
    { revalidate: WIKI_REVALIDATE_SECONDS, tags: ["wiki-index"] }
  );
  return cached();
}
...
export async function loadWikiRelatedData(page: WikiPageContent): Promise<WikiRelatedData> {
  ...
  const catalogPagesBySlugPromise = slug
    ? safeList("wiki collection pages by slug", () => listPublishedWikiCollectionPagesByWikiSlug(slug))
    : Promise.resolve([]);
  ...
  const [
    ...
    safeList("wiki collection pages", () => listPublishedWikiCollectionPagesByUniverseId(universeId)),
...
function mergeCatalogPages(left: WikiCollectionListEntry[], right: WikiCollectionListEntry[]): WikiCollectionListEntry[] {
  const seen = new Set<string>();
  const merged: WikiCollectionListEntry[] = [];
