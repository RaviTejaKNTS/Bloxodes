...
const COLLECTION_FOCUS: Record<string, string> = {
  ...
  "ghost-models": "each model's look and gender, and the hunt tells that come from the body rather than the ghost type",
  challenges: "each Job Site modifier's effect, base cash reward, duration when sourced, and how it changes the hunt",
  ...
  "active-income-methods": "how each manual cash method is triggered, how often it pays, what it is worth in income-per-second terms, and what stays after a reset",
  ...
  "ascension-rewards": "what each Ascension grants, whether it repeats, how it stacks, and what stays locked or off-limits",
  ...
  chores: "board tasks, start objects, room routes, and how to finish each chore between knocks",
  currency: "currency type, earn route, shop use, availability, repeatability, and spending priority",
  ...
  difficulties: "reward multipliers, hunt energy thresholds, fuse box, cursed objects, evidence rules, and hunt behavior",
  ...
  evolutions: "base units, evolved results, result rarity, signature evolution items, item sources, and availability notes",
  familiars: "familiar rarity, access route, equipped unit, element buffs, stat ranges, corruption notes, and reroll status",
  ...
  "fishing-gear": "rod, bobber, and bait upgrades, shop prices, rarity, restock notes, and when to use each fishing loadout",
  ...
  "island-keys": "island, how each key is obtained, what it unlocks, and whether that island is always available",
  ...
  killers: "cost, pressure style, powers, strengths, limits, and chase role",
  ...
  memorias: "rarity, base damage, base range, specialty units, passives, evolution costs, and evolution materials",
  ...
  "orchard-items": "Token costs, fertilizer vs plot-upgrade type, what each item affects, unlock, and when to use it",
  "orchard-mutations": "polarity, affected stat, displayed multiplier, additive value, stacking rule, when the effect applies, obtainment, and whether to keep or replace the roll",
  ...
  natures: "which stat each Nature raises, which stat it cuts, and which roles that trade-off suits",
  ...
  perks: "side, role, triggers, level values, cooldowns, status effects, and playstyle fit",
  "photo-rewards": "journal photo captions, 1-star through 3-star cash payouts, what to photograph, and which evidence or tool each shot uses",
  ...
  props: "prop categories, names, and collection images for roleplay setup",
  ...
  ranks: "RT ranges, attached titles, first-time cash, spins, or glove rewards, and league order",
  ...
  "shiny-relics": "Shiny Relic locations, area hints, progress rewards, and route notes",
  ...
  units: "rarity, element, battlefield type, form status, passives, costs, placement limits, and upgrade count",
...
const FIELD_LABELS: Record<string, string> = {
  ...
  indexUnlockChance: "index unlock chance",
  mechanicsSummary: "mechanics",
  entryItem: "entry item",
  eggNumber: "egg number",
  extraUnlock: "extra unlock",
  firstReward: "first reward",
  evolvedUnit: "evolved unit",
  evolutionStage: "evolution stage",
  evolutionCost: "evolution cost",
  evolutionMaterials: "evolution materials",
  evolutionMaterialCount: "evolution materials",
  evolvable: "evolvable",
  exp: "EXP",
  ...
  equippedUnit: "equipped unit",
  exclusivePets: "exclusive pets",
  exclusiveUnits: "specialty units",
  family: "family",
  ...
  hugeTitanicNote: "Huge / Titanic note",
  health: "Health",
  hostility: "hostility",
  hp: "HP",
  hpBonus: "HP Bonus",
  ...
  mainMechanic: "main mechanic",
  mapName: "map",
  majorRewards: "major rewards",
  ...
  notableAnimals: "notable animals",
  ...
  overshieldPercentage: "overshield",
  ...
  passive: "passive",
  passiveNames: "passives",
  passiveSummary: "passive",
  lockedPassiveNames: "locked passives",
  lockedPassiveSummary: "locked passive effects",
  petClass: "pet class",
  petPool: "pet pool",
  power: "Power",
  ...
  requirementSourceStatus: "requirement source status",
  ...
  setPrice: "set price",
  signatureEvolutionItems: "signature evolution items",
  signatureItemSources: "signature item sources",
  signatureMove: "signature move",
  statRangeSummary: "stat range",
  ...
  spendingPriority: "spending priority",
  slimesNeeded: "slimes needed",
  spinChance: "spin chance",
  stackNote: "stacking note",
  stacking: "stacking",
  grantRule: "grant rule",
  restrictionOrNote: "restriction / note",
  status: "status",
  ...
  officialDiscoveryBadge: "discovery badge",
  weatherOrSecretNote: "weather / secret note",
...
export const GAME_COLLECTIONS: GameCollectionConfig[] = GAME_COLLECTION_GROUPS.flatMap(
  (group) =>
...
export function buildGameCollectionCode(gameSlug: string, collectionSlug: string): string {
  return `${gameSlug}-${collectionSlug}`;
}

export function buildGameCollectionPath(code: string): string {
  const config = getGameCollectionConfigByCode(code);
  if (!config) return `/wiki/${code.trim().toLowerCase()}`;
  return `/wiki/${config.gameSlug}/${config.slug}`;
}

export function getGameCollectionConfigByCode(code: string): GameCollectionConfig | null {
  const normalized = code.trim().toLowerCase();
  return GAME_COLLECTIONS.find((entry) => entry.code === normalized) ?? null;
}

export function getGameCollectionConfigByWikiPath(
  gameSlug: string,
  collectionSlug: string
): GameCollectionConfig | null {
  const normalizedGameSlug = gameSlug.trim().toLowerCase();
  const normalizedCollectionSlug = collectionSlug.trim().toLowerCase();
  return (
    GAME_COLLECTIONS.find(
...
export function buildGameCollectionCopy({
  config,
  itemCount,
  columns,
  imageUrls
}: GameCollectionCopyInput): GameCollectionCopy {
  const countLabel = itemCount.toLocaleString("en-US");
  const override = buildGameCollectionCopyOverride({
    config,
    itemCount,
    countLabel,
    imageUrls
...
Blank fields are left empty instead of padded with guesses. That matters because Roblox game collections rarely share one perfect shape; a pet, boss, vehicle, consumable, quest, and material all need different details to make sense.`;
  ...
  return {
    ...
    cta_label: `Open ${lowerLabel} collection`,
    cta_url: buildGameCollectionPath(config.code),
...
function buildGameCollectionCopyOverride({
  config,
  itemCount,
  countLabel,
  imageUrls
}: Omit<GameCollectionCopyInput, "columns"> & { countLabel: string }): GameCollectionCopy | null {
  if (config.gameSlug === "grow-a-garden-2") {
    return buildGrowGarden2CollectionCopyOverride({ config, itemCount, countLabel, imageUrls });
  }

  if (config.gameSlug === "slime-rng") {
    return buildSlimeRngCollectionCopyOverride({ config, itemCount, countLabel, imageUrls });
  }
  ...
  if (config.code === "wizard-alchemy-potions") {
    ...
    return {
      ...
      cta_label: "Open potions collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-races") {
    ...
    return {
      ...
      cta_label: "Open races collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-wands") {
    ...
    return {
      ...
      cta_label: "Open wands collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-brooms") {
    ...
    return {
      ...
      cta_label: "Open brooms collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-robes") {
    ...
    return {
      ...
      cta_label: "Open robes collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-wizard-hats") {
    ...
    return {
      ...
      cta_label: "Open wizard hats collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-enemies") {
    ...
    return {
      ...
      cta_label: "Open enemies collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-chests") {
    ...
    return {
      ...
      cta_label: "Open chests collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-enchantments") {
    ...
    return {
      ...
      cta_label: "Open enchantments collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-locations") {
    ...
    return {
      ...
      cta_label: "Open locations collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-npcs") {
    ...
    return {
      ...
      cta_label: "Open NPCs collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  if (config.code === "wizard-alchemy-resource-nodes") {
    ...
    return {
      ...
      cta_label: "Open resource nodes collection",
      cta_url: buildGameCollectionPath(config.code),
      wiki_md:
  ...
  return {
    ...
    cta_label: "Open materials collection",
    cta_url: buildGameCollectionPath(config.code),
    wiki_md:
...
function buildGrowGarden2CollectionCopyOverride({
  config,
  itemCount,
  countLabel,
  imageUrls
  ...
  const base = {
    code: config.code,
    cta_label: `Open ${config.label.toLowerCase()} collection`,
    cta_url: buildGameCollectionPath(config.code),
    wiki_sort_order: config.sortOrder,
    wiki_item_count: itemCount,
    thumb_url: imageUrls[0] ?? null
  ...
  const copies: Record<
    string,
    Omit<GameCollectionCopy, keyof typeof base> & {
  ...
  > = {
    seeds: {
      ...
      description_md:
        "## How to choose seeds for your garden\n\nStart with the Sheckles cost and harvest type. Cheap multi-harvest plants are usually easier to keep running because they produce again after the first harvest, while single-harvest plants need a fresh seed each time.\n\nThen compare the obtainment route. Seed Shop plants are the normal upgrade path, while Ghost Pepper Pack plants should be treated as separate pack targets instead of regular Sheckles purchases.",
      how_it_works_md:
      ...
      wiki_md:
        "Grow a Garden 2 seeds cover Seed Shop prices, rarity, harvest type, and Ghost Pepper Pack plants. They are the first collection to check before planning a crop route."
    },
    crops: {
    ...
    gears: {
      ...
      faq_json: [
        ...
        },
        {
          ...
          a: "Yes. Sprinklers appear in the gear list, but they also have a dedicated sprinkler collection because their farming effects matter enough to compare separately."
        },
        {
    ...
    shops: {
      ...
      faq_json: [
        {
          q: "Is there a Guild Rewards collection for Grow a Garden 2?",
        ...
        },
        {
          ...
          a: "Sprinklers are tied to the Gears Shop, and they also have their own collection because their growth and mutation effects are important."
        }
      ],
      wiki_md:
...
function buildSlimeRngCollectionCopyOverride({
  config,
  itemCount,
  countLabel,
  imageUrls
  ...
  switch (config.code) {
    ...
    case "slime-rng-power-fruits": {
      ...
      return {
        ...
        description_md:
          "## How Power Fruits work\n\nPower Fruits fall from meteors after the system is unlocked through the Upgrade Tree. Each fruit adds a power to a slime, such as Lightning, Fire, Ice, Sword, Magician, or Universe. Known fruit entries list two abilities for most fruits, while Universe Fruit still has unknown ability text.\n\n## How to choose a fruit target\n\n1. Check the spawn chance so you know how rare the fruit is compared with the others.\n2. Read the known abilities before giving the fruit to an important slime.\n3. Put a fruit on a slime you plan to keep, because normal free switching is not listed as available.\n4. Avoid using fruited slimes as crafting ingredients, since they are listed as incompatible with the crafting machine.\n5. Claim meteor fruits quickly because the listed claim window is only 2-3 minutes.\n\n## What to do with unknown ability data\n\nWhen a fruit has unknown ability text, the safest comparison is spawn chance and power name. Do not assume the missing ability works like another fruit until the in-game description is clearer.",
        how_it_works_md:
