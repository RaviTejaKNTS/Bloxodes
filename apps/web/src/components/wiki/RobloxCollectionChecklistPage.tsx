...
import { RobloxCollectionChecklist, type RobloxCollectionChecklistSection } from "@/components/wiki/RobloxCollectionChecklist";
import type { GameCollectionDataset } from "@/app/(site)/wiki/collections/games/generic";
import type { WikiCollectionPageContent } from "@/lib/wiki-collections";
...
): RobloxCollectionChecklistSection[] {
...
export function renderRobloxCollectionChecklistPage({
  page,
  config,
  dataset,
  groupedSections,
  ...
  const image = sections.flatMap((section) => section.items).find((item) => item.image)?.image ?? page.thumb_url;
  const schema = [
    ...
    breadcrumbJsonLd([
      ...
      { name: config.gameName, url: `${SITE_URL}/wiki/${config.gameSlug}` },
  ...
  return (
    <div className="catalog-surface space-y-10">
      ...
      <header className="space-y-4">
        <PageBreadcrumb items={[{ label: "Home", href: "/" }, { label: "Wiki", href: "/wiki" }, { label: config.gameName, href: `/wiki/${config.gameSlug}` }, { label: config.label, href: null }]} />
      ...
      {contentHtml?.id ? <CommentsSection entityType="wiki_collection" entityId={contentHtml.id} /> : null}
    </div>
  );
}
