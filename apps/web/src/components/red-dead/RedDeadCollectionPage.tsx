...
import { RedDeadCollectionChecklist, type RedDeadCollectionSection } from "@/components/red-dead/RedDeadCollectionChecklist";
import type { GameCollectionDataset } from "@/app/(site)/wiki/collections/games/generic";
import type { RedDeadWikiCollectionPage } from "@/lib/red-dead";
...
export function renderRedDeadCollectionPage({
  page,
  config,
  dataset,
  groupedSections,
  ...
  const canonicalPath = `/red-dead/wiki/${config.gameSlug}/${config.slug}`;
  ...
  const schema = [
    ...
    breadcrumbJsonLd([
      ...
      { name: config.gameName, url: `${SITE_URL}/red-dead/wiki/${config.gameSlug}` },
  ...
  return (
    <div className="catalog-surface space-y-10">
      ...
      <header className="space-y-4">
        <PageBreadcrumb items={[{ label: "Home", href: "/" }, { label: "Red Dead", href: "/red-dead" }, { label: "Wiki", href: "/red-dead/wiki" }, { label: config.gameName, href: `/red-dead/wiki/${config.gameSlug}` }, { label: config.label, href: null }]} />
      ...
      {contentHtml?.id ? <CommentsSection entityType="red_dead_wiki_collection" entityId={contentHtml.id} /> : null}
    </div>
  );
}
