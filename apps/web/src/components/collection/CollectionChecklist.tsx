...
export function CollectionChecklist({
  ...
  const { session, checked, toggle, reset } = useCollectionChecklistProgress(progressOptions);
  ...
  const statusText = session.userId ? "Saved to your account" : "Saved in this browser";
      ...
      <div className="rounded-lg border border-border/70 bg-surface p-5 md:p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="min-w-0">
            <p className="text-sm text-muted">Mark each entry as you find it.</p>
            <p className="mt-1 text-sm font-medium text-foreground" role="status" aria-live="polite">
              {done} of {total} found · {percent}% complete
            </p>
          </div>
          <p className="text-xs text-muted" title={statusText}>{statusText}</p>
        </div>
                      ...
                      <label id={`item-${item.id}`} key={item.id} className={`group flex min-w-0 cursor-pointer flex-col overflow-hidden rounded-lg border bg-surface transition hover:border-accent/60 ${isChecked ? "border-accent/50" : "border-border/70"}`}>
                        <div className="flex min-w-0 items-start gap-3 p-4">
                          <input
                            ...
                            className="sr-only"
                          />
                          <span aria-hidden className={`mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-md border transition ${isChecked ? "border-accent bg-accent text-white" : "border-border/80 bg-background text-transparent group-hover:border-accent/70"}`}>
                        ...
                        <div className="relative aspect-[16/9] w-full border-y border-border/60 bg-background/40">
                          {image ? (
                            ...
                            <div className="flex h-full items-center justify-center px-4 text-center text-sm text-muted">Image unavailable</div>
                          )}
                        </div>
                        {fieldKeys.length ? (
