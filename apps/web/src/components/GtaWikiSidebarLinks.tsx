...
type GtaWikiSidebarLinksProps = {
  pathname: string;
  onNavigate?: () => void;
};

export function GtaWikiSidebarLinks({ pathname, onNavigate }: GtaWikiSidebarLinksProps) {
  return (
    <nav className="mt-3 border-t border-sidebar-border/80 pt-2" aria-label="GTA game wikis">
      <div className="h-7 px-2.5 text-[11px] font-semibold leading-7 text-sidebar-foreground/45">
        Game wikis
      </div>
      <ul className="space-y-0.5">
        {gtaWikiNavLinks.map((link) => {
          const current = pathname === link.href;
          const active = isGtaWikiNavLinkActive(pathname, link.href);
          return (
            <li key={link.href}>
              <Link
                href={link.href}
              ...
              >
                <span
                  aria-hidden
                  className={cn(
                    "h-4 w-0.5 shrink-0 rounded-full bg-transparent",
