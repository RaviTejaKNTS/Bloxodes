...
export type CatalogPaginationSection<TItem extends CatalogPaginationItem> = {
  id: string;
  label: string;
  items: TItem[];
...
export type CatalogPaginationInfo = {
  currentPage: number;
  totalPages: number;
  totalItems: number;
...
export type CatalogPaginationSectionLink = {
  id: string;
  label: string;
  count: number;
  page: number;
...
type PageBucket<TItem extends CatalogPaginationItem> = {
  sections: CatalogPaginationSection<TItem>[];
  weight: number;
  itemCount: number;
};
...
function estimateItemWeight(item: CatalogPaginationItem) {
  let textLength = 0;
...
function sectionWeight<TItem extends CatalogPaginationItem>(section: CatalogPaginationSection<TItem>) {
  return section.items.reduce((sum, item) => sum + estimateItemWeight(item), 0);
}

function cloneSectionWithItems<TItem extends CatalogPaginationItem>(
  section: CatalogPaginationSection<TItem>,
  items: TItem[],
  isContinuation = false
): CatalogPaginationSection<TItem> {
  return {
    ...section,
    items,
...
function pushBucket<TItem extends CatalogPaginationItem>(buckets: PageBucket<TItem>[], bucket: PageBucket<TItem>) {
  if (!bucket.sections.length) return;
  buckets.push(bucket);
}

function splitLargeSection<TItem extends CatalogPaginationItem>(
  section: CatalogPaginationSection<TItem>,
  targetWeight: number
): PageBucket<TItem>[] {
...
export function buildCatalogPagination<TItem extends CatalogPaginationItem>({
  sections,
  currentPage,
  basePath,
  ...
  const sectionLinks = sections.map((section) => {
    ...
    return {
      ...
      page,
      href
    } satisfies CatalogPaginationSectionLink;
  });
  ...
  return {
    ...
    info: {
      ...
      pageStartIndex,
      basePath
    } satisfies CatalogPaginationInfo
  };
}
