...
export function CatalogImageLightbox({ containerId }: { containerId: string }) {
  ...
  useEffect(() => {
    ...
    const onClick = (event: MouseEvent) => {
      ...
      const trigger = target?.closest<HTMLButtonElement>("[data-catalog-image-preview]");
      if (!trigger || !container.contains(trigger)) return;

      const src = trigger.dataset.catalogImageSrc;
      if (!src) return;

      setImage({
        src,
        alt: trigger.dataset.catalogImageAlt || "Catalog item image"
      });
    };
