...
async function buildCatalogContent(systemSlug: string) {
...
export default async function AdminCommandSystemPage({ params }: { params: Promise<{ system: string }> }) {
  ...
  const introNodes = introHtml ? renderPageContentNodes(introHtml, "admin-system-intro") : null;
  const descriptionNodes = descriptionHtml.flatMap((entry) =>
    renderPageContentNodes(entry.html, `admin-system-description-${entry.key}`)
  );
  const howNodes = howHtml ? renderPageContentNodes(howHtml, "admin-system-how") : null;
  const faqNodes = faqHtml.map((faq, idx) => ({
    ...faq,
    nodes: renderPageContentNodes(faq.a, `admin-system-faq-${idx}`)
  }));
    ...
    <div className="catalog-surface space-y-10">
      <PageBreadcrumb
        items={[
          ...
          { label: "Admin Commands", href: "/catalog/admin-commands" },
          ...
          <div className="flex flex-wrap gap-2">
            {sections.map((section) => (
              <a
                key={section.id}
                href={`#${section.id}`}
                className="rounded-md border border-border/70 bg-background/70 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-muted transition hover:border-accent/70 hover:text-accent"
              >
                {section.title}
              </a>
            ))}
