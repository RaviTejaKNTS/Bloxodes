...
import { SellLemonsOfflineIncomeCalculatorClient } from "./SellLemonsOfflineIncomeCalculatorClient";

export const revalidate = 21600;

const TOOL_CODE = "sell-lemons-offline-income-calculator";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = resolveSeoTitle(tool?.seo_title) ?? tool?.title ?? "Sell Lemons Offline Income Calculator";
  const description =
    tool?.meta_description ??
    "Estimate Sell Lemons offline earnings from your income rate, offline time, multipliers, and target cash.";
...
export default async function SellLemonsOfflineIncomeCalculatorPage() {
  const tool = await getToolContentWithDevFallback(TOOL_CODE);
  ...
  const structuredData = {
    ...
    "@graph": [
      {
        "@type": "WebPage",
        name: tool?.title ?? "Sell Lemons Offline Income Calculator",
        description:
          tool?.meta_description ??
          "Estimate Sell Lemons offline earnings from your income rate, offline time, multipliers, and target cash.",
        url: CANONICAL,
        ...
        breadcrumb: {
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 3, name: tool?.title ?? "Sell Lemons Offline Income Calculator" }
          ]
        },
        mainEntity: {
          "@type": "WebApplication",
    ...
    <>
      ...
      <PageBreadcrumb
        ...
        items={[
          ...
          { label: tool?.title ?? "Sell Lemons Offline Income Calculator", href: null }
        ]}
      />

      <header className="space-y-3">
        <h1 className="text-4xl font-semibold leading-tight text-foreground md:text-5xl">
          {tool?.title ?? "Sell Lemons Offline Income Calculator"}
        </h1>
      ...
      <section id="article-body" itemProp="articleBody" className="article-content md-copy-scope copy-with-sidebar-space mt-8 space-y-6 journey-content-stream journey-content-stream--interactive">
        ...
        <ContentSlot
          ...
          fullWidthResponsive
        />
        <SellLemonsOfflineIncomeCalculatorClient />
