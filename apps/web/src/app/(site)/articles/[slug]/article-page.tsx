...
import { parseArticleContentPages, type RenderableArticleContentBlock } from "@/lib/article-blocks";
...
type RenderedArticleBlock =
  ...
  | { kind: "roblox-game-card"; data: Parameters<typeof RobloxGameCard>[0]["data"] }
  | { kind: "invalid" };

async function renderArticleContent(blocks: RenderableArticleContentBlock[]): Promise<RenderedArticleBlock[]> {
  return Promise.all(
...
export function articleContentPageCount(contentMd: string): number {
  return parseArticleContentPages(contentMd).length;
}

export async function generateArticlePageMetadata(slug: string, pageNumber = 1): Promise<Metadata> {
  const article = await getArticleBySlug(slug);
  if (!article || !Number.isSafeInteger(pageNumber) || pageNumber < 1) return {};

  const totalPages = articleContentPageCount(article.content_md);
  if (pageNumber > totalPages) return {};

  const canonicalPath = pageNumber > 1 ? `/articles/${article.slug}/page/${pageNumber}` : `/articles/${article.slug}`;
  ...
  const pageTitle = pageNumber > 1 ? `${title} - Page ${pageNumber}` : title;
  ...
  return {
    ...
    category: article.universe?.display_name ?? article.universe?.name ?? "Gaming",
    openGraph: {
      type: "article",
      url: canonicalUrl,
      title: pageTitle,
...
export async function renderArticlePage(article: ArticleWithRelations, currentPage = 1) {
  const contentPages = parseArticleContentPages(article.content_md);
  const totalPages = contentPages.length;
  const safePage = Math.min(Math.max(1, currentPage), totalPages);
  const currentBlocks = contentPages[safePage - 1] ?? [];
  const showEndMatter = safePage === totalPages;
  const canonicalPath = safePage > 1 ? `/articles/${article.slug}/page/${safePage}` : `/articles/${article.slug}`;
  const canonicalUrl = `${SITE_URL}${canonicalPath}`;
  const coverImage = article.cover_image?.startsWith("http")
    ? article.cover_image
    : article.cover_image
  ...
  const faqEntries = showEndMatter ? normalizeArticleFaqEntries(article.faq_json) : [];
  const [articleBlocks, authorBioHtml, faqHtml] = await Promise.all([
    renderArticleContent(currentBlocks),
  ...
  return (
    <div className="space-y-12">
      ...
      <article className="min-w-0">
        ...
        >
          {articleBlocks.map((block, index) => {
            ...
            if (block.kind === "roblox-game-card") {
              return <RobloxGameCard key={`roblox-game-card-${block.data.id}`} data={block.data} />;
            }
            return (
              <div
                key={`invalid-${index}`}
        ...
        {totalPages > 1 ? (
          <div className="mt-10 space-y-3 border-t border-border/60 pt-6">
            <p className="text-sm font-semibold text-muted">
              Continue reading: page {safePage} of {totalPages}
            </p>
            <PagePagination
        ...
        {showEndMatter && faqItems.length ? (
          <ContentFaq items={faqItems} className="mt-10 border-t border-border/60 pt-6" />
        ) : null}

        {showEndMatter && article.author ? (
      ...
      {showEndMatter ? <MoreArticles excludeSlug={article.slug} /> : null}
    </div>
  );
}
