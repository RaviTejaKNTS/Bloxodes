...
import {
  ...
  getRedDeadWikiPageBySlug,
  listPublishedRedDeadWikiCollectionImageUrls,
  listPublishedRedDeadWikiCollectionsByWikiSlug,
  resolveRedDeadWikiCoverImage,
  resolveRedDeadWikiThumbnailImage
} from "@/lib/red-dead";
...
import type { RedDeadWikiPage } from "@/lib/red-dead";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const page = await getRedDeadWikiPageBySlug(slug);
  const canonical = `${SITE_URL}${buildRedDeadWikiPath(slug)}`;
  ...
  const image = resolveRedDeadWikiCoverImage(page);
  return {
    title,
    description,
...
export default async function RedDeadWikiDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const page = await getRedDeadWikiPageBySlug(slug);
  if (!page) notFound();

  const collections = await listPublishedRedDeadWikiCollectionsByWikiSlug(page.slug);
  ...
  const coverImage = resolveRedDeadWikiCoverImage(page);
  const thumbnailImage = resolveRedDeadWikiThumbnailImage(page);
  const platforms = stringList(page.game_platforms_json);
  const releases = releaseLabels(page.game_release_dates_json);
  const canonicalPath = buildRedDeadWikiPath(page.slug);
  ...
  const collectionBlocks = await Promise.all(collections.map(async (collection) => {
    const [copyHtml, imageUrls] = await Promise.all([
      ...
      listPublishedRedDeadWikiCollectionImageUrls(collection, 6)
    ]);
  ...
  const structuredData = [
    ...
    breadcrumbJsonLd([
      ...
      { name: "Red Dead", url: `${SITE_URL}/red-dead` },
      { name: "Red Dead Wiki", url: `${SITE_URL}/red-dead/wiki` },
  ...
  return (
    <div className="space-y-9">
      ...
      <header className="space-y-6">
        <PageBreadcrumb items={[{ label: "Home", href: "/" }, { label: "Red Dead", href: "/red-dead" }, { label: "Wiki", href: "/red-dead/wiki" }, { label: page.game_short_title || page.game_title, href: null }]} />
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,2.2fr)_minmax(20rem,1fr)]">
        <article id="article-body" itemProp="articleBody" className="min-w-0 space-y-9 journey-content-stream journey-content-stream--prose">
          ...
          {releases.length || platforms.length || page.game_developer || page.game_publisher ? (
            <section aria-label="Game details" className="lg:hidden">
              <RedDeadGameDetails page={page} releases={releases} platforms={platforms} />
            </section>
          ) : null}

          {collectionGroups.map((group) => (
            <section key={group.key} className="space-y-6" aria-labelledby={`red-dead-${group.key}-heading`}>
              <h2 id={`red-dead-${group.key}-heading`} className="text-2xl font-semibold leading-tight text-foreground">{group.label}</h2>
              <div className="space-y-6">
                {group.entries.map(({ collection, copyHtml, imageUrls }) => (
                  <section key={collection.id} className="space-y-4" data-journey-item>
                    ...
                    <WikiCollectionCta
                      href={buildRedDeadCollectionPath(collection.wiki_slug, collection.collection_slug)}
        ...
        <div className="min-w-0 lg:col-start-1">
          <CommentsSection entityType="red_dead_wiki" entityId={page.id} />
        </div>
      </div>
    </div>
  );
...
function RedDeadGameDetails({ page, releases, platforms }: { page: RedDeadWikiPage; releases: string[]; platforms: string[] }) {
  const items = [
