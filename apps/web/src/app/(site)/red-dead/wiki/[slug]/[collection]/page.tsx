...
import { renderRedDeadCollectionPage } from "@/components/red-dead/RedDeadCollectionPage";
import type { GameCollectionRenderConfig } from "@/lib/game-collections";
import {
  buildRedDeadCollectionPath,
  getPublishedRedDeadWikiCollectionRuntime,
  getRedDeadWikiCollectionPageByPath,
  listPublishedRedDeadWikiCollectionsByWikiSlug,
  type RedDeadWikiCollectionPage
} from "@/lib/red-dead";
...
async function resolveContext(slug: string, collection: string): Promise<Context | null> {
  ...
  const page = await getRedDeadWikiCollectionPageByPath(wikiSlug, collectionSlug);
  if (!page) return null;
  const runtime = await getPublishedRedDeadWikiCollectionRuntime(page);
  if (!runtime) throw new Error(`Required Red Dead database runtime for ${page.code} did not load.`);
  return {
    page,
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateRedDeadCollectionMetadata({ slug, collection, currentPage: 1 });
}

export async function generateRedDeadCollectionMetadata({
  slug,
  collection,
  currentPage
}: {
  ...
  const page = await getRedDeadWikiCollectionPageByPath(normalize(slug), normalize(collection));
  const basePath = buildRedDeadCollectionPath(slug, collection);
  ...
  const runtime = await getPublishedRedDeadWikiCollectionRuntime(page);
  const titleBase = resolveSeoTitle(page.seo_title) ?? page.title ?? `Red Dead Wiki | ${SITE_NAME}`;
...
export default async function RedDeadCollectionPage({ params }: PageProps) {
  const { slug, collection } = await params;
  return renderRedDeadCollectionPageRoute({ slug, collection, currentPage: 1 });
}

export async function renderRedDeadCollectionPageRoute({
  slug,
  collection,
  currentPage
}: {
  ...
  const [contentHtml, collections] = await Promise.all([
    ...
    listPublishedRedDeadWikiCollectionsByWikiSlug(context.page.wiki_slug)
  ]);
  const collectionOptions = collections.map((entry) => ({
    value: entry.code,
    label: entry.display_name,
    href: buildRedDeadCollectionPath(entry.wiki_slug, entry.collection_slug),
    pageType: entry.page_type
  }));
  if (context.page.page_type === "collectible") {
    return renderRedDeadCollectionPage({
      page: context.page,
  ...
  // The shared renderer currently owns Roblox/GTA comment entity types. Render
  // its collection body without an id, then attach the Red Dead comment target
  // here so this route remains isolated from those namespaces.
  ...
  const renderedCollection = renderGameCollectionPage({
    ...
    routeBase: "/red-dead/wiki",
    wikiLabel: "Red Dead Wiki",
    collectionOptions,
    ...
    <>
      ...
      {contentHtml?.id ? (
        <div className="mx-auto mt-10 w-full max-w-5xl px-4 sm:px-6 lg:px-8">
          <CommentsSection entityType="red_dead_wiki_collection" entityId={contentHtml.id} />
        </div>
      ) : null}
    </>
  );
...
export async function getRedDeadCollectionPageCount(slug: string, collection: string): Promise<number> {
  const context = await resolveContext(slug, collection);
  if (!context) return 1;
