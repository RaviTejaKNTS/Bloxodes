...
- Extension auth opens `/api/extension/auth/complete` in `chrome.identity.launchWebAuthFlow`; an explicit confirmation exchanges a redirect-bound, short-lived, one-time code for an `app_sessions` bearer token. Session and logout routes accept that bearer token.
