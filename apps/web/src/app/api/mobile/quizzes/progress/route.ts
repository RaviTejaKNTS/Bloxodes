...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (code) {
      ...
      if (error) {
        return NextResponse.json({ error: "Unable to load quiz progress." }, { status: 500, headers });
      }
      ...
      return NextResponse.json(
        {
          seenQuestionIds: normalizeQuestionIds(row?.seen_question_ids),
          lastScore: typeof row?.last_score === "number" ? row.last_score : null,
          lastTotal: typeof row?.last_total === "number" ? row.last_total : null,
          lastBreakdown: normalizeBreakdown(row?.last_breakdown),
    ...
    console.error("Failed to load mobile quiz progress", error);
    return NextResponse.json(
      { error: "Unable to load quiz progress." },
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const code = normalizeCode(typeof payload?.code === "string" ? payload.code : null);
    ...
    if (!code) {
      return NextResponse.json({ error: "Quiz code is required." }, { status: 400, headers });
    }
    ...
    if (seenQuestionIds.length === 0) {
      ...
      if (error) {
        return NextResponse.json({ error: "Unable to clear quiz progress." }, { status: 500, headers });
      }

      return NextResponse.json({ seenQuestionIds: [] }, { headers });
    }
    ...
    if (error) {
      return NextResponse.json({ error: "Unable to save quiz progress." }, { status: 500, headers });
    }

    return NextResponse.json({ seenQuestionIds }, { headers });
  } catch (error) {
    console.error("Failed to save mobile quiz progress", error);
    return NextResponse.json(
      { error: "Unable to save quiz progress." },
      { status: 500, headers: mobileCredentialedFallbackHeaders(METHODS) }
    );
  }
}
