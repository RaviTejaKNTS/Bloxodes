...
The remaining worker-update follow-up is complete. After explicit user approval, the dedicated VPS public key was registered as a verified read-only GitHub deploy key, the worker checkout was pinned to that identity through an SSH host alias, and private `production` branch access succeeded. The unmodified nightly `build-image.sh` path then fetched the latest production commit and rebuilt the worker image, so future production releases no longer require a manual snapshot transfer.
