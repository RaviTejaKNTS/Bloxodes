...
- Development defaults to true local Supabase. Managed development and production preview require named profiles; production and tests are process-only.
...
`npm run env:check` then confirmed 119 stored variable names were covered by 17 committed example files and checked private-file permissions. Legacy source files remain available in the main checkout and the pre-migration backup; only the isolated worktree links were removed.
