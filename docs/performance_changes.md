...
- **Deploy warmup:** After deployment and any broad purge, run `CACHE_WARM_SITE_URL=https://bloxodes.com npm run cache:warm`, or manually dispatch the Dokploy production workflow with `trigger_dokploy=false` after a Dokploy-only manual deploy. The default deploy mode warms index/legal pages, all wiki/catalog/tool pages, sitemap files, and only a recent slice from Supabase-heavy detail sitemaps. Use `CACHE_WARM_MODE=full` only when a full-site warm is intentional.
