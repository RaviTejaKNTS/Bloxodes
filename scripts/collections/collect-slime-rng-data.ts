...
async function collectPowerFruits(): Promise<PowerFruitRow[]> {
  ...
  for (const [index, match] of listMatches.entries()) {
    ...
    rows.push({
      name,
      slug: slugify(name),
      collectionSection: "Power Fruits",
      spawnChance,
...
async function main() {
  ...
  await writeDataset(
    ...
    ["family", "variant", "collectionSection", "rarity", "odds", "power", "health", "image"],
    [source("Slimes")],
    slimes
  );
  ...
  await writeDataset(
    ...
    ["collectionSection", "spawnChance", "power", "abilityOne", "abilityTwo", "upgradeNote", "restrictions", "image"],
    [
      source("Power_Fruits"),
  ...
  await writeDataset(
    ...
    ["rebirthNumber", "rebirthRange", "collectionSection", "goopRequired", "luckMultiplier", "resetNote"],
    [source("Rebirths")],
    rebirths
  );
