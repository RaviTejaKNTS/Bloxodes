...
async function collectSocialLinksFromExistingSources(game: GameRow): Promise<PlaceholderLinks> {
...
async function enrichGame(game: GameRow, stats: EnrichmentStats): Promise<void> {
  ...
  const { error } = await supabase.from("games").update(updatePayload).eq("id", game.id);
...
async function countWhereNotNull(column: keyof GameRow): Promise<number> {
...
async function verify() {
  const { count: total, error: totalError } = await supabase
    ...
    .select("id", { count: "exact", head: true })
    .eq("is_published", false);
  if (totalError) throw new Error(`Failed to count games: ${totalError.message}`);
...
async function main() {
  ...
  const games = await loadUnpublishedGames(limit, startAfterSlug);

  if (!games.length) {
    console.log("No local unpublished games found.");
    return;
  }
  ...
  await promisePool(games, concurrency, async (game) => {
    try {
      await enrichGame(game, stats);
    } catch (error) {
      stats.failed += 1;
