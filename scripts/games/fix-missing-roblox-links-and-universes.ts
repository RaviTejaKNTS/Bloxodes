...
function printUsage() {
  console.log(`Usage: npm run fix:game-links -- [options]

Repairs games that have intro_md but are missing both roblox_link and universe_id.
...
Options:
  ...
  --limit <count>          Max matching games to process. Defaults to all matches.
  --concurrency <count>    Number of games to process at once. Defaults to ${DEFAULT_CONCURRENCY}.
  --prod                   Use .env directly instead of local development env.
  --published-only         Only process published games.
...
async function collectSocialLinksFromExistingSources(game: GameRow): Promise<RepairLinks> {
...
function shouldSetLink(game: GameRow, column: LinkColumn, options: CliOptions): boolean {
...
async function loadGames(options: CliOptions): Promise<GameRow[]> {
  const selectFields =
...
async function processGame(game: GameRow, options: CliOptions, stats: RepairStats): Promise<void> {
  stats.processed += 1;
...
async function main() {
  ...
  const games = await loadGames(options);

  if (!games.length) {
    console.log(options.slug ? `No game found for slug "${options.slug}".` : "No matching games found.");
    return;
  }

  console.log(
    `${options.apply ? "Applying" : "Dry run for"} ${games.length} game${games.length === 1 ? "" : "s"} against ${
  ...
  await promisePool(games, options.concurrency, async (game) => {
    try {
      await processGame(game, options, stats);
    } catch (error) {
      stats.failed += 1;
