...
export function assignStatsTier(input: TierInput): { tier: StatsTier; reason: string; refreshHours: number } {
  ...
  if (!input.lastStatsRefreshedAt || (playing == null && visits == null)) {
    return { tier: "NEW", reason: "new_or_missing_stats", refreshHours: 2 };
  }
  if ((playing ?? 0) >= 100) {
    return { tier: "HOT", reason: "playing_gte_100", refreshHours: 1 };
  }
  if ((visits ?? 0) >= 250_000_000) {
    return { tier: "HOT", reason: "visits_gte_250m", refreshHours: 1 };
  }
  if ((playing ?? 0) >= 30) {
    return { tier: "WARM", reason: "playing_gte_30", refreshHours: 12 };
  }
