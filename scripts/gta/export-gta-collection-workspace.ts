...
async function main() {
  ...
  const { data: gameData, error: gameError } = await sb
    ...
    .eq("id", row.game_id)
    .maybeSingle();
  if (gameError) throw new Error(`Failed to read GTA game: ${gameError.message}`);
  if (!gameData?.title) throw new Error(`Missing GTA game for ${row.code}.`);
  const finalJson = {
    wiki_slug: row.wiki_slug,
    collection_slug: row.collection_slug,
    code: row.code,
    display_name: row.display_name ?? row.collection_slug,
    ...
    wiki_md: row.wiki_md ?? "",
    wiki_sort_order: row.wiki_sort_order ?? 0,
