...
import {
  getGameDatasetCatalogConfigByWikiPath,
...
function printUsage() {
  console.log(`Usage:
  npm run check:game-catalog-data -- --game <game-slug> --collection <collection-slug> [options]
...
async function main() {
  ...
  const config = getGameDatasetCatalogConfigByWikiPath(options.game!, options.collection!);
  if (!config && !options.file) {
    throw new Error(
      `No registered catalog config for ${options.game}/${options.collection}. Run register:game-catalog or pass --file.`
    );
  }

  const datasetPath = options.file
    ? resolvePath(options.file)
    : repoPath("data", (config as GameDatasetCatalogConfig).dataDir, (config as GameDatasetCatalogConfig).file);
  ...
  if (!sectionField) {
    issues.push({ level: "error", message: "No useful section field found. Add meta.groupBy or catalogSection values." });
  ...
  if (options.json) {
    ...
    console.log(`Game catalog data check: ${options.game}/${options.collection}`);
