...
const CONFIG_PATH = repoPath("apps", "web", "src", "lib", "game-dataset-catalogs.ts");

function printUsage() {
  console.log(`Usage:
  npm run register:game-catalog -- --game <game-slug> --collection <collection-slug> [--dry-run]

Adds a collection slug to an existing GAME_DATASET_CATALOG_GROUPS game entry.
...
async function main() {
  ...
  if (getGameDatasetCatalogConfigByWikiPath(options.game!, options.collection!)) {
  ...
  if (!bounds) {
    console.log(`No GAME_DATASET_CATALOG_GROUPS entry found for ${options.game}. Add a group like:\n`);
