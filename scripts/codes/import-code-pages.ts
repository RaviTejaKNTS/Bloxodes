...
import { computeCodePageDetails, syncCodePageCodesFromSources } from "@/lib/admin/code-page-import";
...
async function importSingleCodePage(
  ...
  const { slug: derivedSlug, name: derivedName } = computeCodePageDetails({
    slug,
    name,
    sourceUrl
  });
  assertEditorialSlug(derivedSlug, "code_pages.slug");
  ...
  const { data: codePage, error: upsertError } = await sb
    .from("code_pages")
    .upsert(upsertPayload, { onConflict: "slug" })
    .select("id, slug, name, source_url, source_url_2, source_url_3, is_published")
    .single<CodePageRecord>();

  if (upsertError || !codePage) {
    throw new Error(upsertError?.message ?? "Upsert failed");
  }

  const syncResult = await syncCodePageCodesFromSources(sb, codePage.id, [
    codePage.source_url,
    codePage.source_url_2,
    codePage.source_url_3,
  ]);

  if (syncResult.errors.length) {
    console.warn(`⚠ Failed to import codes for ${codePage.slug}: ${syncResult.errors.join(", ")}`);
  }

  return {
    slug: codePage.slug,
    name: codePage.name,
    publish: Boolean(codePage.is_published),
...
async function main() {
  try {
    ...
    for (const entry of entries) {
      stats.processed += 1;
      try {
        const result = await importSingleCodePage(sb, entry);
        stats.success += 1;
