...
async function generateInterlinkingForGame(params: {
  ...
  const { data: targetRow, error: targetError } = await supabase
    ...
    .maybeSingle();
  if (targetError) {
    console.warn("⚠️ Failed to load target code page for interlinking:", targetError.message);
    return null;
  }
  ...
  if (publishedError) {
    console.warn("⚠️ Failed to load published code pages for interlinking:", publishedError.message);
    return null;
  }
...
function sourceUrlsFromFields(game: ExistingCodePageRecord, fields: readonly ResearchSourceField[]): string[] {
  return uniqueUrls(fields.flatMap((field) => splitSourceUrls(game[field])));
}

function articleSourceUrls(game: ExistingCodePageRecord): string[] {
  return sourceUrlsFromFields(game, ARTICLE_SOURCE_FIELDS);
}

function hasArticleSourcesBeyondPrimary(game: ExistingCodePageRecord): boolean {
  return articleSourceUrls(game).length > 0;
}

function sourceFieldCount(game: ExistingCodePageRecord, field: ArticleSourceField): number {
...
async function loadDraftCodePages(limit: number, slug: string | null): Promise<ExistingCodePageRecord[]> {
  if (slug) {
    ...
    if (error) throw new Error(`Failed to load draft code pages: ${error.message}`);
...
async function loadDraftCodePagesWithArticleSources(): Promise<ExistingCodePageRecord[]> {
  const codePages: ExistingCodePageRecord[] = [];
  for (let from = 0; ; from += DRAFT_CODE_PAGE_SIZE) {
    const to = from + DRAFT_CODE_PAGE_SIZE - 1;
    const { data, error } = await supabase
      .from("code_pages")
      .select(CODE_PAGE_ARTICLE_SELECT)
      .eq("is_published", false)
    ...
    if (error) throw new Error(`Failed to load sourced draft code pages: ${error.message}`);
    const batch = ((data ?? []) as ExistingCodePageRecord[])
      .filter((game) => Boolean(game.id && game.name && game.slug))
      .filter(hasArticleSourcesBeyondPrimary);
    codePages.push(...batch);
    if ((data ?? []).length < DRAFT_CODE_PAGE_SIZE) break;
  }
  return codePages;
}

async function collectArticleSources(game: ExistingCodePageRecord): Promise<string> {
...
async function collectSocialLinksFromExistingSources(game: ExistingCodePageRecord): Promise<PlaceholderLinks> {
...
async function processDraftCodePage(game: ExistingCodePageRecord): Promise<void> {
...
async function main() {
  ...
  console.log(`🧾 Processing ${codePages.length} draft code page${codePages.length === 1 ? "" : "s"}...`);
  let completed = 0;
  let failed = 0;

  for (const game of codePages) {
    try {
      await processDraftCodePage(game);
      completed += 1;
    } catch (error) {
      failed += 1;
