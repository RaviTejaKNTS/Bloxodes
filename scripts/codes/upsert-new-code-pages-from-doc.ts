...
const DOC_PATH = "docs/New Code Pages To Import.md";
const READY_SECTION = "Ready to Import";
const UPDATE_SECTION = "Add to Existing Code Pages";
...
function printUsage() {
  console.log(`Usage: npx tsx scripts/codes/upsert-new-code-pages-from-doc.ts [--apply]

Reads ${DOC_PATH} and:
- inserts rows from "${READY_SECTION}" as draft code pages
...
async function loadExistingCodePages(): Promise<ExistingCodePage[]> {
  const sb = supabaseAdmin();
  const pageSize = 1000;
  const codePages: ExistingCodePage[] = [];
  ...
  for (let from = 0; ; from += pageSize) {
    ...
    const chunkRows = (data ?? []) as ExistingCodePage[];
    codePages.push(...chunkRows);
...
async function main() {
  ...
  const existingCodePages = await loadExistingCodePages();

  const existingBySlug = new Map(existingCodePages.map((row) => [row.slug, row]));
  const existingCodePageByUrl = new Map<string, ExistingCodePage>();
  for (const row of existingCodePages) {
    for (const url of [row.source_url, row.source_url_2]) {
      if (url) existingCodePageByUrl.set(url, row);
    }
  }
  ...
  const readyUrlConflicts = readyPayloads.flatMap((row) => {
    ...
    for (const url of [row.source_url, row.source_url_2]) {
      if (url && existingCodePageByUrl.has(url)) conflicts.push(url);
    }
  ...
  if (readySlugConflicts.length > 0 || readyUrlConflicts.length > 0) {
    throw new Error(
      `Ready payload conflicts with existing code pages. Slug conflicts: ${readySlugConflicts.length}, URL conflicts: ${readyUrlConflicts.length}`
    );
  }
  ...
  const updatePayloads: UpdatePayload[] = updates.map((row) => {
    ...
    for (const [field, url] of [
      ...
      if (!url) continue;
      const owner = existingCodePageByUrl.get(url);
      if (owner && owner.slug !== row.slug) {
        throw new Error(`${field} URL already belongs to another code page: ${url} -> ${owner.slug}`);
      }
    }
