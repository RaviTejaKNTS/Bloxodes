...
function printUsage() {
  console.log(`\nUsage: npm run upsert:code-page -- --file <payload.json> [options]\n\nOptions:\n  --file <path>          JSON payload with code_pages fields and source URLs.\n  --dry-run              Print the planned create/update without writing.\n  --draft                Save the code page as unpublished.\n  --publish              Save the code page as published.\n  -h, --help             Show this help message.\n`);
}
...
async function upsertCodePage(payload: CodePagePayload, options: CliOptions) {
  const sb = supabaseAdmin();
  const slug = slugify(stripCodesSuffix(payload.slug ?? payload.name));
  assertEditorialSlug(slug, "code_pages.slug");
  ...
  const { data: existing, error: existingError } = await sb
    ...
    .eq("slug", slug)
    .maybeSingle<ExistingCodePage>();

  if (existingError) {
    throw new Error(`Failed to check existing code page: ${existingError.message}`);
  }
  ...
  const codePagePayload: Record<string, unknown> = {
    name: payload.name,
    slug,
  ...
  if (options.dryRun) {
    ...
    console.log(JSON.stringify({ codePagePayload }, null, 2));
    return;
  }

  const query = existing
    ? sb.from("code_pages").update(codePagePayload).eq("id", existing.id)
    : sb.from("code_pages").insert(codePagePayload);
  ...
  if (saveError || !saved) {
    throw new Error(`Failed to ${existing ? "update" : "create"} code page row: ${saveError?.message ?? "no row returned"}`);
  }
...
async function main() {
  ...
  await upsertCodePage(payload, options);
}
