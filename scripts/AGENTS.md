...
- `catalog/`: Roblox item and bundle collection plus enrichment.
  ...
  - `register-game-catalog.ts` safely adds a collection slug to an existing `GAME_DATASET_CATALOG_GROUPS` game entry. Use `npm run register:game-catalog -- --game <game-slug> --collection <collection-slug> --dry-run` first. If the game group is missing, it prints the manual group block instead of editing.
  - `check-game-catalog-data.ts` validates local game catalog datasets before writing page copy: registered config, item rows, section field/counts, useful card fields, card summaries, local image paths, and optional `description_json` section-key matches. Use it before final writing and it is also called by `verify-game-catalog-finals`.
  - `collect-catalog-images.ts` downloads item images from a manifest into `apps/web/public/<Game>/<Collection>` and can update matching dataset `image` fields. Use `--dry-run` first and prefer clean item/wiki images over edited thumbnails.
...
- `content/`: local content QA helpers.
  ...
  - `verify-wiki-final.ts`, `verify-game-catalog-finals.ts`, and `verify-catalog-finals.ts` wrap the approved seed scripts with copy checks, local table readback, and route checks before Browser preview. Game catalog verification also runs `check:game-catalog-data` for every collection before seeding.
...
- `universes/`: universe collection, enrichment, stats route slugs, tiered stats, media, and descriptions.
  ...
  - `rebuild-stats-current-indexes.ts` calls `refresh_stats_current_indexes()` to rebuild the public stats read indexes, then queues broad stats revalidation; use `npm run stats:index:refresh`.
