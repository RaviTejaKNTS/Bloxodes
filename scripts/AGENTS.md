...
- `content/`: local content QA helpers.
  ...
  - Article writing is owned by `.agents/skills/bloxodes-article-workflow-runner`: research subagent → parent brief approval → **writing subagent** (`bloxodes-article-writing` or `bloxodes-tech-article-writing`) → parent review → `verify:article-finals`.
