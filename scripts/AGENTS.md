...
- `universes/`: universe collection, enrichment, stats route slugs, tiered stats, media, and descriptions.
  ...
  - `search-roblox-universes.ts` is the Roblox omni-search discovery collector. It uses keyword/prefix searches, fetches official game details only to resolve identity, and inserts missing rows into `roblox_universes` as `NEW`; it does not replace the stats refresh/tier workflow.
  - `expand-roblox-creators.ts` discovers public games from creators/groups behind existing HOT/WARM universes. It inserts missing rows into `roblox_universes` as `NEW` and leaves existing universe rows untouched.
