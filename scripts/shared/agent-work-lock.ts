...
export async function acquireAgentWorkLock(
  ...
  const lockDir = path.join(worktree, "tmp", "agent-work");
  const lockPath = path.join(lockDir, "agent.lock");
  ...
  throw new Error(`Could not acquire shared agent-work lock at ${lockPath}.`);
}
