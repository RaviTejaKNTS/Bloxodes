...
```bash
npm run articles:review:list -- --base-url http://127.0.0.1:<port> --limit 100 --json
```
...
1. Re-query every selected queue ID and require `workflow_mode = agent_runner` and `status = completed`. Keep `ARTICLE_DEV_SUPABASE_*` pointed at managed dev; production commands must not replace them.
...
```bash
npm run articles:queue:update -- --queue-id <uuid> --status published --production-url https://bloxodes.com/articles/<slug> --apply
```
...
```bash
npm run articles:queue:update -- --queue-id <uuid> --status rejected --reason "<reason>" --apply
```
