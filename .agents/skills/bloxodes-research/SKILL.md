...
When the user asks only what catalog pages can be made for a game, do catalog-only discovery instead of full game coverage. Check production `wiki_catalog_pages` and relevant public wiki catalog routes first, check global `catalog_pages` only for overlap, research trusted online sources for durable in-game collections, and return a clean `[create]`, `[we already have a page]`, or `[skip]` catalog list. Do not audit or recommend codes, articles, tools, wiki, checklist, quiz, events, gamepasses, badges, developer products, servers, raw Roblox media, or broad platform metadata unless needed to reject a catalog duplicate or weak idea.
