---
name: bloxodes-game-catalog-images
description: Gather, save, wire, and verify images for one approved Bloxodes game catalog after data approval. Use for item image source checks, image manifests, local public image paths, dataset image fields, image coverage, and image readiness before catalog writing. Do not write final.json.
---

# Bloxodes Game Catalog Images
...
9. Run the readiness check with images required when this catalog should have images:

```bash
npm run check:game-catalog-data -- --game <game-slug> --collection <collection-slug> --require-images
```

Use `--file <dataset.json>` if the catalog is not registered yet.
...
If images are important and coverage is weak, stop and say what is missing. If the catalog can work without images, explain why and mark the missing images as accepted.
