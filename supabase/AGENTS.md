...
- Production Supabase is self-hosted on the same VPS as the web app. The production API endpoint is `https://bloxodesdb.ravitejaknts.com`; Studio is `https://bloxodesstudio.ravitejaknts.com`. Managed Supabase is a rollback/source fallback, not the hot production path.
