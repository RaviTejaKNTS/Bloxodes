-- Repair production drift: the migration ledger included 20260918000012, but
-- wiki_pages_view still had the older definition. Recreate only this read view
-- so it matches the application contract without changing source tables.

drop view if exists public.wiki_pages_view;

create view public.wiki_pages_view
