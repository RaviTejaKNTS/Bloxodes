...
- Collection parent handoffs contain only the skill, game, and collection names; workspace context is inherited. Collection skills build useful supported coverage, record unresolved rows/values for later improvement, and block only material too weak to form a worthwhile page. Source count disagreements and incomplete current rosters are not automatic blockers. The image pass gathers and wires available exact-item images even when coverage is partial.
...
- Scheduled jobs stage and verify managed development, then publish their exact hub and collection manifests directly to production and verify live URLs. `--skip-production-release` remains available for explicit previews. The builder and Codex keep the restricted `bloxodes-wiki-model` account and development/media credentials. A separate trusted publisher service runs as `teja`, reads the protected production env, and never starts model turns. It processes only exact live leased queue requests and returns success/errors in `production_receipt`; the builder retains its heartbeat while waiting and resumes its session to repair failures.
- The CLI session ID and a maximum of three repair turns are persisted in `session.json` under the queue artifact root. Technical failures resume that same session with the captured, secret-redacted error; code repeats the idempotent failed sync/publication. Queue retries reuse `result_root` and completed artifacts. The lease heartbeat remains active through publication; the session is marked complete only after production readback/live verification. Historical ephemeral runs can reuse completed artifacts but cannot resume a missing CLI session.
- Both development and production publication normalize oversized media with the existing publisher option. `WIKI_AUTOMATION_QUEUE_ID` selects one existing queue row for recovery without choosing another game.
...
September 9 recovery verification: Southern Mudding OffRoading completed automatic production publication through the separate publisher, including oversized-media normalization, exact hub/collection URL checks, sitemap inclusion, and published queue acknowledgement. Three recovery tests and a real restricted CLI session-resume canary passed. Legacy ephemeral artifacts were reused without regenerating content.
