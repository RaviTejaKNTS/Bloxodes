...
- The verified production database contains 663 published collection pages with valid dataset pointers and 46,732 published item rows, including the 15 pre-existing GTA 5 collections. The other 158 reviewed GTA collection promotions and 19 approved Roblox checklist-type conversions remain pending until the guarded production schema publisher can authenticate to the VPS and apply the `page_type` migration.
