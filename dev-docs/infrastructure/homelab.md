...
- `bloxodes-article-writer.service`: triggered after successful discovery; readiness requires authenticated Codex using the configured `gpt-5.6-luna`/`xhigh` contract, Pi pinned to GPT-5.6 Luna at max reasoning, plus a real headless Playwright/Chrome smoke test before the queue batch starts. Codex owns research/images/review, while Pi owns prose. The workflow uses `verify:article-browser` for rendered localhost QA, so it does not depend on a desktop browser bridge being attached. If a provider, preview, or release step fails, the batch returns unfinished processing claims to retryable `blocked` rows instead of leaving them stuck until stale-claim recovery.
- `bloxodes-wiki-builder.timer`: runs daily at `01:00` local time without catch-up. Enabling it after that time cannot start an unplanned game, and the oneshot service prevents overlap when a prior run is still active.
...
Wiki runtime env is `/etc/bloxodes/wiki-automation.env`, root-owned, group-readable only by `bloxodes-wiki-model`, mode 640. It contains managed-dev Supabase, the bucket-scoped shared `bloxodes-wiki` R2 credential, and fixed Luna Max settings. The model account cannot traverse the checkout `.envs` directory or the operator home.

Every wiki Codex child runs directly in the shared checkout as the dedicated `bloxodes-wiki-model` account. Normal filesystem permissions keep tracked code and `.envs` read-only or inaccessible; only `tmp/wiki-automation` and the lane-specific `.next/wiki-automation` cache are writable. The trusted `teja` operator has inherited read-only access to result artifacts so the separate publisher can validate them without running as root. Readiness proves the production target is inaccessible and the independent Codex login works before the service starts. Completed rows stop at `managed_dev_ready`; production publication remains a separate reviewed operation.
...
- The release step promotes only the claimed queue game's exact wiki final and runtime manifests, rechecks production identity, verifies existing R2 objects, publishes database pointers, and polls every live hub/collection URL before marking the queue row published.
