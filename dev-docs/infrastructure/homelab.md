...
## Wiki automatic publication and recovery (September 9, 2026)

The wiki service now supports a separate trusted operator-account publisher with the builder/model still restricted, persistent CLI sessions, bounded same-session technical repair, consistent image normalization, and automatic production publication of the exact completed game. The daily cadence and shared article/wiki lock remain unchanged. `--skip-production-release` is manual preview mode; normal scheduled execution publishes after development verification. Earlier managed-development-only descriptions above are historical. The publisher timer checks live leased requests each minute without acquiring the model work lock. It has no model execution path and returns errors through queue receipts for the builder to repair. Activation verified September 9: the builder remains `bloxodes-wiki-model`, publisher is `teja`, and both have empty capability sets. Both timers are active. Southern Mudding OffRoading reused its completed artifacts, normalized its oversized image, and reached published after live hub, Vehicles, and sitemap verification. Its historical run was ephemeral, so this recovery needed no model turn; a separate two-turn restricted CLI canary verified saved-session resumption and recall. Recovery failure-injection tests passed. Publication now polls for sitemap cache propagation before failing. Temporary exact-game overrides were removed.
...
Persistent article and wiki artifacts live in the shared runtime `state` directory. The old article-runtime state path and the primary checkout's `tmp/wiki-automation` remain compatibility symlinks so saved absolute artifact paths continue to resolve. The shared `tmp/article-writer` lease remains anchored in the primary checkout and linked into each runtime. Protected `.envs` remains linked from the primary checkout; the restricted wiki model cannot read it. The model writes only its home, wiki artifacts, the shared lease and the release's ignored `.next` cache. The trusted publisher keeps its separate operator identity and existing production target.
