...
Every wiki Codex child runs directly in the shared checkout as the dedicated `bloxodes-wiki-model` account. Normal filesystem permissions keep tracked code and `.envs` read-only or inaccessible; only `tmp/wiki-automation` and the lane-specific `.next/wiki-automation` cache are writable. Readiness proves the production target is inaccessible and the independent Codex login works before the service starts. Completed rows stop at `managed_dev_ready`; production publication remains a separate reviewed operation.
