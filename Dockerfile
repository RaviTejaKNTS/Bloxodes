...
USER nextjs
EXPOSE 3000
HEALTHCHECK --interval=30s --timeout=10s --start-period=30s --retries=5 CMD node -e "fetch('http://127.0.0.1:3000/api/health?scope=deploy').then((res)=>{if(!res.ok)process.exit(1);}).catch(()=>process.exit(1))"
