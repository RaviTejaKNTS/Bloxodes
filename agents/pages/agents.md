...
| Stats | `/stats`, `/stats/games`, `/stats/games/[slug]`, `/stats/creators`, `/stats/items`, `/stats/items/[assetId]` | `src/app/(site)/stats/*`, shared read layer in `src/lib/stats.ts`, chart UI in `stats/components/*`, backed by `roblox_universes`, stats current-index tables, hourly/daily stats tables, rank snapshot tables, `roblox_catalog_items`, and item stats hourly/daily/resale tables |
