...
| Quizzes | `/quizzes`, `/quizzes/[slug]` | `src/app/(site)/quizzes/*`, scoped guide in `src/app/(site)/quizzes/AGENTS.md`, shared helpers in `src/app/(site)/quizzes/page-data.tsx`. Use `$bloxodes-quiz-writing` for metadata and question pools stored in `quiz_pages.quiz_data`. Detail pages should keep the user in the quiz experience: use the page intro plus the interactive quiz, not a separate "what this quiz covers" about block. |
...
| Tower Defense Simulator wiki | `/wiki/tower-defense-simulator`, `/wiki/tower-defense-simulator/towers`, `/wiki/tower-defense-simulator/modes` | Generic wiki hub and collection routes backed by `data/Tower Defense Simulator/`; maps and enemies remain research-blocked and unregistered |
