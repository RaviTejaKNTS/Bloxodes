...
| Area | Routes | Files / Notes |
| --- | --- | --- |
| Codes | `/codes`, `/codes/page/[page]`, `/codes/[slug]` | `src/app/(site)/codes/*`, shared helpers in `src/app/(site)/codes/page-data.tsx`, evergreen landing guidance in `index-content.tsx`, and CollectionPage/ItemList structured data. The guide appears only on page one; continuation pages are self-canonical and `/codes/page/1` redirects to `/codes`. Slugs are game slugs only, not `<game>-codes`; code rows come from `scripts/codes/update-codes.ts` using RobloxDen in `source_url` and Beebom in `source_url_2`. |
...
| Wiki | `/wiki`, `/wiki/page/[page]`, `/wiki/[slug]` | `src/app/(site)/wiki/*`, detail helpers in `page-data.tsx`, gaming reference tables in `index-content.tsx`, overview composition in `index-page-data.tsx`, stats/collections/events readers in `src/lib/wiki-index.ts`, and a server-filtered 20-card directory with self-canonical continuation pages and noindex query variants; image-role resolver in `src/lib/wiki-images.ts`, and Supabase read layer in `src/lib/wiki.ts`. Index cards/social previews use landscape universe thumbnails while detail title artwork uses the newest official square universe icon. |
| GTA | `/gta`, `/gta/wiki`, `/gta/wiki/[slug]`, `/gta/wiki/[slug]/[collection]`, paginated collection routes | `src/app/(site)/gta/*`, server-only read layer in `src/lib/gta.ts`, GTA-specific sidebar/search scope, a newest-first direct sidebar list of all 16 released game wikis on desktop and mobile, and platform-owned `gta_*` tables; collection rendering reuses the generic Bloxodes wiki collection surface. GTA VI and GTA tools stay absent until they are ready. |
| Red Dead | `/red-dead`, `/red-dead/wiki`, `/red-dead/wiki/[slug]`, `/red-dead/wiki/[slug]/[collection]`, paginated database collection routes | `src/app/(site)/red-dead/*`, server-only read layer in `src/lib/red-dead.ts`, Red Dead-specific sidebar/search scope, progress API for collectible collections, and platform-owned `red_dead_*` tables; database and collectible collections share one route family. |
...
GTA standalone checklist routes: `/gta/checklists`, `/gta/checklists/page/<page>`, `/gta/checklists/<game-slug>`. Shared checklist template/board/cards; managed development currently publishes GTA V, San Andreas, Vice City, and GTA Online at their game slugs. Includes GTA navigation/search, wiki and collection links, metadata, feed, GTA sitemap and revalidation.
