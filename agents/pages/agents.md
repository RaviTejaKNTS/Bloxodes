...
| Area | Routes | Files / Notes |
| --- | --- | --- |
| Codes | `/codes`, `/codes/page/[page]`, `/codes/[slug]` | `src/app/(site)/codes/*`, shared helpers in `src/app/(site)/codes/page-data.tsx`, evergreen landing-page guidance in `index-content.tsx`, and CollectionPage/ItemList structured data. Full guide only on page one; continuation pages are self-canonical and `/codes/page/1` redirects to `/codes`. Slugs are game slugs only, not `<game>-codes`; code rows come from `scripts/codes/update-codes.ts` using RobloxDen in `source_url` and Beebom in `source_url_2`. |
