...
| Discover recent non-codes article leads | `scripts/articles/discover-article-topics.ts` | `npm run articles:discover` for a dry run; homelab managed-dev writes use `-- --apply` and stage raw, source-deduplicated candidates without creating generation jobs |
| Curate source leads into article jobs | `scripts/articles/curate-article-topics.ts` | `npm run articles:curate` for a read-only Groq preview; homelab managed-dev writes use `-- --apply`. Only Groq-approved guides, tier lists, and explainers reach the `agent_runner` queue; same-topic sources are grouped into one row and production overlap comes from the GET-only editorial inventory. |
...
| Update an article-runner queue row | `scripts/articles/update-article-queue-item.ts` | `npm run articles:queue:update -- --queue-id <uuid> --status <processing|completed|skipped|failed> --apply`; the command accepts only managed-dev article credentials and completion also requires `--result-path <final.json>` |
...
| Install inactive homelab article units | `scripts/ops/install-homelab-article-automation.sh` | Run with sudo only from `/srv/data/bloxodes-article-worker/current`; installs root-owned env placeholders and systemd units but leaves both timers disabled until readiness passes |
...
| Audit stats workflow | `scripts/universes/audit-universe-stats-workflow.ts` | `npm run stats:audit`; reports stale stored player values at the 24-hour public cutoff and seven-day threshold, and records `stats_universe_audit` in `stats_job_runs` |
...
| Refresh universe stats | `scripts/universes/update-universe-hourly-stats.ts` | Tiered NEW/HOT/WARM/COLD refresh with atomic due-row claims, tier SLAs, exponential error backoff, and recoverable quarantine after three confirmed missing Roblox detail responses |
...
| Audit universe stats | `scripts/universes/audit-universe-stats-workflow.ts` | Reports tier/data coverage plus active and expired leases, retry backoff, unavailable cooldowns, and refresh-SLA adoption; expired leases record a partial run |
