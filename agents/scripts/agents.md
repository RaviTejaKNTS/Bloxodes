...
| Migrate or verify quiz payloads | `scripts/quizzes/sync-quiz-data.ts` | `npm run sync:quiz-data -- --export <manifest.json>` or `--input <manifest.json>`; normal reviewed quiz finals use `import:content-final`, which writes and verifies `quiz_pages.quiz_data` |
...
| Collect Grow a Garden game collection images | `scripts/collections/collect-grow-a-garden-images.ts` | `npm run collect:grow-a-garden-images` |
...
| Collect game collection images from manifest | `scripts/collections/collect-collection-images.ts` | `npm run collect:collection-images -- --manifest <images.json> --dataset data/<Game>/<collection>.json --game-name "<Game>" --collection-name "<Collection>" --dry-run` first; updates flat or v2 `items[].system.image` fields |
