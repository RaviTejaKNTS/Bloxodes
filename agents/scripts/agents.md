...
| Build one top-100 game wiki and its defensible collections | `scripts/wiki/run-homelab-wiki-automation.ts` | `npm run wiki:homelab:run` previews the next durable candidate; the scheduled `-- --apply --skip-production-release` run writes only managed development/shared R2, requires a verified hub plus at least one collection, and stops at `managed_dev_ready` |
