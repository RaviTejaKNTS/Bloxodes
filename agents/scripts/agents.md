...
| Publish immutable collection runtime | `scripts/collections/sync-game-collection-runtime.ts` | `npm run sync:game-collection-runtime -- --game <game-slug> --collection <collection-slug> --normalize-legacy-media --upload-media --apply --publish`; managed development by default, validates database/R2 readback, and is required before route preview because runtime has no local fallback |
