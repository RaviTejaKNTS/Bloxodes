...
| Audit universe stats | `scripts/universes/audit-universe-stats-workflow.ts` | Uses `get_roblox_universe_pipeline_health()` for a single-statement snapshot of tier/data coverage, public 24-hour visibility, recent NEW/WARM/COLD worker starts, leases, overdue rows, and read-index freshness; `--strict` exits non-zero on failed SLAs |
