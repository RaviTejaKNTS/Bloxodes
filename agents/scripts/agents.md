...
| Sync one GTA collection revision | `scripts/gta/sync-gta-collection-runtime.ts` | `npm run sync:gta-collection-runtime -- --manifest <runtime-manifest.json>` for a dry plan; manifests may set `collection.pageType` to `database` or `checklist` (default `database`); add `--apply --upload-media --publish` only after final approval in managed development |
...
| Publish immutable collection runtime | `scripts/collections/sync-game-collection-runtime.ts` | `npm run sync:game-collection-runtime -- --manifest <runtime-manifest.json> --normalize-legacy-media --upload-media --apply --publish`; managed development by default, validates database/R2 readback, and uses `collection.pageType` (`database` or `checklist`, default `database`) to select the public renderer before route preview |
