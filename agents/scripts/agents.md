...
| Write a capped managed-dev article batch on the homelab | `scripts/articles/run-homelab-article-batch.ts` | `npm run articles:writer:batch` checks the queue without invoking a writer; add `-- --apply --limit 6` to requeue due blocked rows and run the parent workflow with GPT-5.6 Luna at max. Research/images/review use Codex and prose uses Pi; there is no alternate-model fallback. A non-empty batch with zero completed rows exits degraded. |
| Validate homelab article automation | `scripts/ops/check-homelab-article-automation.ts` | `npm run articles:homelab:check` is read-only and checks the dev schema, production inventory, Groq curation key, authenticated Codex Luna Max, Pi 0.84.3+ with ChatGPT auth and Luna Max settings, and a browser before the timer is enabled. |
