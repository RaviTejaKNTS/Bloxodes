...
| Write a capped managed-dev article batch on the homelab with Codex and Grok fallback | `scripts/articles/run-homelab-article-batch.ts` | `npm run articles:writer:batch` checks the queue without invoking a writer; add `-- --apply --limit 6` to requeue due blocked rows and run Codex with GPT-5.6 Luna at xhigh reasoning. One Grok 4.5 fallback is permitted only for classified Codex provider/account failures, with already active queue slots excluded. A non-empty batch with zero completed rows exits degraded. Systemd triggers it only after successful discovery and curation. |
