...
| Update an article-runner queue row | `scripts/articles/update-article-queue-item.ts` | `npm run articles:queue:update -- --queue-id <uuid> --status <processing|completed|published|rejected|skipped|failed> --apply`; completion requires `--result-path`, publication requires `--production-url`, and rejection requires `--reason` |
