...
| Write and auto-release a capped article batch on the homelab | `scripts/articles/run-homelab-article-batch.ts` | `npm run articles:writer:batch` checks the queue without invoking a writer; add `-- --apply --limit 6` to select exact pending IDs, run Codex with GPT-5.6 Luna at xhigh reasoning, and publish only the rows completed by that batch through the guarded production release. One Grok 4.5 fallback is permitted only for classified Codex provider/account failures, with already active queue slots excluded. Use `--skip-production-release` for an intentional manual-release run. A non-empty batch with zero completed rows exits degraded. Systemd triggers it only after successful discovery and curation. |
