...
| Run daily top-100 wiki coverage | `scripts/wiki/run-homelab-wiki-automation.ts` | `npm run wiki:homelab:run -- --apply --skip-production-release`; the installed service forces one Luna Max lane and one claimed game per run. Approved temporary batches may use two lanes with a shared `WIKI_AUTOMATION_MAX_GAMES_PER_RUN` cap. Use `--apply --release-only` from the trusted operator context to publish ready rows without claiming a new game. |
...
| Release exact completed articles | `scripts/articles/release-completed-articles.ts` | `npm run articles:release -- --queue-id <uuid> [--queue-id <uuid> ...]` plans an exact allowlist; add `--apply --allow-prod` to promote approved media, import rows, sync provenance, poll live pages, and close only verified queue rows. It never scans every completed row. |
...
| Audit production article copy for source/process leaks | `scripts/content/audit-production-article-copy.ts` | `npm run articles:audit-copy:production -- --days 30`; read-only production scan, with `--all` for the full inventory and `--all-rules` for broader copy checks |
| Run only the isolated article prose stage with Pi | `scripts/articles/run-pi-article-writer.ts` | `npm run articles:writer:pi -- --skill <writing-skill> --title <title> --slug <slug> --type <type> --workspace <path>` is a dry run; add `--apply` after approved `brief.md` and `media.json`. It hard-pins ChatGPT-authenticated Pi to `openai-codex/gpt-5.6-luna` at max, limits tools and context to the article workspace, then runs copy and image-readiness checks. |
| Verify Pi Luna Max model access | `scripts/articles/run-pi-writer-canary.ts` | `npm run articles:writer:pi:canary` is a dry run; add `-- --apply` for one tiny authenticated exact-response canary. |
| Write and release a capped homelab article batch | `scripts/articles/run-homelab-article-batch.ts` | `npm run articles:writer:batch` checks the queue without invoking a writer; add `-- --apply --limit 6` to run the Luna Max workflow and automatically release only the exact rows completed by that batch after the model exits. Use `--skip-production-release` for manual review. A zero-completion or release failure exits degraded. |
| Validate homelab article automation | `scripts/ops/check-homelab-article-automation.ts` | `npm run articles:homelab:check` is read-only and checks the dev schema, production inventory, guarded production release target when enabled, Groq curation key, authenticated Codex Luna Max, Pi 0.84.3+ with ChatGPT auth and Luna Max settings, and a browser before the timer is enabled. |
...
| Install the pinned homelab Pi writer | `scripts/ops/install-homelab-pi-writer.sh` | Run with sudo only from the exact clean approved homelab SHA; installs Pi 0.84.3 for `teja` without copying or changing auth. Then log in interactively with ChatGPT Plus/Pro. |
...
| Sync article image provenance | `scripts/content/sync-article-image-provenance.ts` | Dry-run `npm run sync:article-image-provenance -- --manifest <media.json>`; add `-- --apply` after article import. Upserts match the article-owned uploaded object path, so repeated source URLs remain separate. Production also requires `NODE_ENV=production` and `--allow-prod` |
...
| Article media checks (YouTube + images) | `scripts/content/check-article-media.ts` | used by `npm run verify:article-finals`; tier-list titles/slugs require one tier-list component, including text-only items, plus cue → table → added-analysis flow per tier |
| Article final writing | `.agents/skills/bloxodes-article-workflow-runner` | Codex Luna Max research → brief approval → mandatory image pass → image readiness approval → Pi Luna Max prose writer → parent review and `verify:article-finals` (sibling `media.json` required) |
...
| Publish/preview a collection runtime revision | `scripts/collections/sync-game-collection-runtime.ts` | `npm run sync:game-collection-runtime -- --manifest <runtime-manifest.json>` is dry-run; managed development uses `BLOXODES_ENV_OVERLAYS=cloudflare` plus `--apply --upload-media --publish`. Production additionally requires `production-preview` and `--allow-prod`; both targets verify the same immutable objects in `bloxodes-wiki`. |
...
| Sync quiz JSONB payloads | `scripts/quizzes/sync-quiz-data.ts` | `npm run sync:quiz-data -- --input <manifest.json>` is dry-run; add `--apply` for managed development. Production additionally requires `NODE_ENV=production --allow-prod`. `--export` is a guarded compatibility backfill path. |
