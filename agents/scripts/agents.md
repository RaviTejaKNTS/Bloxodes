...
| Event guide generation | `scripts/events/generate-events-articles.ts` | `npm run generate:events-articles`; shares the article editorial standard for drafting, revision, linking, and polish, with verified UTC event timestamps rendered ET first. |
...
| Release completed articles to production | `scripts/articles/release-completed-articles.ts` | `npm run articles:release -- --queue-id <uuid> --apply --allow-prod`; exact queue IDs only, with production readback, live URL verification, provenance verification, and queue closure |
...
| Run one explicit article through code-owned stages | `scripts/articles/run-article-pipeline.ts` | `articles:pipeline -- --input <job.json> --run-dir <ignored-run> --apply` runs managed-development research, media, writing, focused reviews and technical QA; no queue or production writes. Without --apply it only plans. |
| Write one curated managed-dev article | `scripts/articles/run-local-article-writer.ts` | `articles:writer:local` and `articles:writer:homelab` delegate to the code-controlled batch with one row and no production release; add `--apply` for queue/content writes. |
| Write and auto-release a capped article batch on the homelab | `scripts/articles/run-homelab-article-batch.ts` | `articles:writer:batch -- --apply --limit 6` claims exact curated rows in code, runs focused Luna max research/images/writing/review stages and automated technical checks, and releases only code-verified completed IDs. `--skip-production-release` keeps managed-dev work for review. Grok is a classified provider-failure fallback for one stage only. |
| Validate homelab article automation | `scripts/ops/check-homelab-article-automation.ts` | `npm run articles:homelab:check` is read-only and checks the dev schema, production inventory, Groq key, authenticated Codex CLI/model configuration, Grok fallback CLI, and browser before the systemd timers are enabled |
...
| Build one top-100 game wiki and its suggested collections | `scripts/wiki/run-homelab-wiki-automation.ts` | `npm run wiki:homelab:run` previews the next durable candidate; collection skills create useful supported coverage from every `[create]` suggestion with minimal parent handoffs; the scheduled `-- --apply` run stages managed development/shared R2, requires a verified hub plus at least one collection, and publishes the exact game to production; saved sessions repair technical failures up to three times |
| Validate homelab wiki automation | `scripts/ops/check-homelab-wiki-automation.ts` | `npm run wiki:homelab:check` validates the managed-dev queue/RPC contract, restricted Codex login, browser, shared R2 canary, and production top-100/inventory inputs with bounded retries |
| Verify the managed-dev wiki queue contract | `scripts/wiki/check-wiki-queue-contract.ts` | `npm run wiki:queue:check -- --apply` creates and removes disposable canary rows to verify targeted claims, two slots, heartbeat security, lease recovery, and attempt exhaustion |
| Install and enable homelab wiki units | `scripts/ops/install-homelab-wiki-automation.sh` | Run with sudo only from the exact clean approved homelab checkout; replaces the legacy same-named unit files and enables the persistent daily timer |
...
| Audit GTA collection authoring quality | `scripts/gta/audit-gta-collection-quality.py` | `npm run audit:gta-collection-quality -- --map <reviewed-authoritative-map.json> --json <report.json>`; read-only signals for empty/duplicate/filler fields, malformed page and FAQ copy, thin rows, missing images with documented procedural-only exceptions, and reused image bytes; source review still decides factual readiness |
| Sync one GTA collection revision | `scripts/gta/sync-gta-collection-runtime.ts` | `npm run sync:gta-collection-runtime -- --manifest <runtime-manifest.json>` for a dry plan; manifests may set `collection.pageType` to `database` or `collectible` (default `database`); add `--apply --upload-media --publish` only after final approval in managed development |
| Reconcile reviewed GTA batch | `scripts/gta/verify-gta-collection-runtime.ts` | `npm run verify:gta-collection-runtime -- --map <authoritative-map.json>`; managed-development-only readback of all published pointers, dataset hashes/counts, and page copy; dry plans are batched in groups of 20 to bound prepared-media memory. Keep the map explicit to avoid stale nested/outer workspace files. |
...
| Sync one non-Roblox franchise collection revision | `scripts/franchise/sync-franchise-collection-runtime.ts` | `npm run sync:franchise-collection-runtime -- --namespace <gta|red-dead> --manifest <runtime-manifest.json>` for a dry plan; add `--apply --upload-media --publish` only after final approval in managed development |
| Verify one non-Roblox franchise wiki hub | `scripts/franchise/verify-franchise-wiki-final.ts` | `npm run verify:franchise-wiki-final -- --namespace <gta|red-dead> --base-url http://localhost:3000 --game <game-slug> --workspace <workspace>`; managed-development upsert, DB readback, and route verification |
| Sync non-Roblox franchise wiki title media | `scripts/franchise/sync-franchise-wiki-media.ts` | `npm run sync:franchise-wiki-media -- --namespace <red-dead> [--apply]`; source-checks reviewed artwork, writes R2-hosted wide `cover_image` and square `hero_image` pointers in managed development only |
| Verify one non-Roblox franchise collection end to end | `scripts/franchise/verify-franchise-collection-final.ts` | `npm run verify:franchise-collection-final -- --namespace <gta|red-dead> --base-url http://localhost:3000 --game <game-slug> --collection <collection-slug> --workspace <workspace>`; managed-development readback and route/page-type verification |
...
| Audit one v2 game collection workspace dataset and renderer | `scripts/collections/audit-game-collection-datasets-v2.ts` | `npm run audit:game-collection-datasets:v2 -- --game <game-slug> --collection <collection-slug> --file tmp/content-workspace/<game-slug>/collections/<collection-slug>/dataset.json`; repository-wide local dataset discovery is retired |
| Export existing collection page copy | `scripts/collections/export-game-collection-final.ts` | `npm run export:game-collection-final -- --game <game-slug> --collection <collection-slug> --output-root tmp/content-workspace/<game-slug>/collections`; read-only managed-development Supabase snapshot, with `--allow-remote-read` required for another target and `--force` required to replace a file |
| Publish immutable collection runtime | `scripts/collections/sync-game-collection-runtime.ts` | `npm run sync:game-collection-runtime -- --manifest <runtime-manifest.json> --normalize-legacy-media --upload-media --apply --publish`; managed development by default, validates database/R2 readback, and uses `collection.pageType` (`database` or `collectible`, default `database`) to select the public renderer before route preview |
| Sync one wiki hub runtime | `scripts/collections/sync-game-wiki-runtime.ts` | `npm run sync:game-wiki-runtime -- --final-json <final.json> --game <game-slug> --universe-id <id>`; requires the official 512x512 universe icon and a 768x432 thumbnail, fetches missing media before apply, and rejects non-null `cover_image` unless the caller explicitly supplies `--allow-cover-override` for a reviewed exception |
| Converge production collection revisions into managed development | `scripts/collections/sync-production-collection-runtime-to-managed-dev.ts` | `npm run sync:production-collection-runtime:managed-dev` is read-only by default; add `-- --apply` to copy missing exact immutable revisions into managed development only while preserving target IDs and production read-only state |
...
| Enrich universes | `scripts/universes/enrich-roblox-universes.ts` | `npm run enrich:universes`, `npm run enrich:universes:light`, `npm run enrich:universes:deep`; each run selects the current official 512x512 icon and ordered 768x432 thumbnails as the active universe media while retaining older URLs in `roblox_universe_media`; deep mode also refreshes Open Cloud metadata, social links, groups, game passes, badges, and recent public server snapshots |
...
| Collect game collection images from manifest | `scripts/collections/collect-collection-images.ts` | `npm run collect:collection-images -- --manifest <images.json> --dataset tmp/content-workspace/<game-slug>/collections/<collection-slug>/dataset.json --game-name "<Game>" --collection-name "<Collection>" --dry-run` first; updates v2 `items[].system.image` fields in the ignored authoring workspace |
| Seed broad catalog pages into Supabase | `scripts/catalog/seed-catalog-pages.ts` | `npm run seed:catalog-pages -- --file tmp/content-workspace/<topic>/catalogs/<batch>/final.json --dry-run`; writes to managed development by default and refuses production unless `--allow-prod` is supplied |
| Refresh game-specific ID source mappings | `scripts/catalog/sync-game-specific-id-sources.ts` | `npm run sync:game-specific-id-sources`; combines official Roblox Music Discovery associations with dedicated game-wiki parsers for 3008, Retail Tycoon 2, and Nico's Nextbots where needed, rewrites `data/game-specific-ids/source-backed.json` from the bounded source adapters, and must be diff-reviewed before seeding. Use `-- --only-music-game <slug>` for a bounded experience-song refresh that preserves other generated groups |
| Seed game-specific ID usage | `scripts/catalog/seed-game-specific-id-usage.ts` | `npm run seed:game-specific-id-usage -- --dry-run` first, then run against managed development; use `--replace-source-rows` to reconcile only rows owned by matching game/source pairs, while other targets require `--allow-prod` |
| Seed gathered game wiki pages into Supabase | `scripts/collections/seed-game-wiki-pages.ts` | `npm run seed:game-wiki-pages`, add `-- --dry-run` to preview |
| Import reviewed content final JSON into Supabase | `scripts/content/import-content-final.ts` | `npm run import:content-final -- --file tmp/content-workspace/<game-or-topic-slug>/<page-folder>/final.json --dry-run`; article imports write only to `articles`, fill missing authors randomly, create edited game-thumbnail covers when needed, keep the feature image in `cover_image` only, reject duplicate cover URLs in `content_md`, and require `/articles` plus `/articles/<slug>` verification against the same saved row. Use `--regenerate-covers` for an intentional fresh-object cover repair. |
...
| Refresh universe stats | `scripts/universes/update-universe-hourly-stats.ts` | Tiered NEW/HOT/WARM/COLD refresh with atomic due-row claims, 20-hour healthy COLD scheduling ahead of the 24-hour public cutoff, exponential error/all-null backoff, and recoverable quarantine after three confirmed missing Roblox detail responses |
...
| Audit universe stats | `scripts/universes/audit-universe-stats-workflow.ts` | Uses the timeout-safe `get_roblox_universe_pipeline_health_v4()` for a single-statement snapshot of tier/data coverage, public 24-hour visibility, recent NEW/WARM/COLD worker starts, universe-only stale runs, leases, overdue rows, and read-index freshness; `--strict` exits non-zero on failed SLAs |
| Import RobloxDen free-item candidates | `scripts/catalog/import-robloxden-free-items.py` | `npm run import:free-item-candidates`; defaults to managed development, other targets require `ALLOW_PROD_FREE_ITEMS_IMPORT=true`, and rows still require the Roblox verification collector before publication |
...
| Collect Roblox font IDs | `scripts/catalog/collect-roblox-font-ids.ts` | `npm run collect:font-ids`; dry-run by default, reads the complete official Creator Store FontFamily set plus manifests and preview thumbnails, and upserts `roblox_font_ids` only with `-- --apply` (`--allow-prod` is also required outside managed development) |
| Collect Roblox mesh IDs | `scripts/catalog/collect-roblox-mesh-ids.ts` | `npm run collect:mesh-ids`; dry-run by default, reads the public 1,000-result Creator Store MeshPart selection plus square previews, and upserts `roblox_mesh_ids` only with `-- --apply` (`--allow-prod` is also required outside managed development) |
...
| Build and smoke the VPS stats-worker image | `scripts/ops/vps-build-stats-worker.sh`, `scripts/ops/smoke-stats-worker.ts` | Release only an exact approved SHA. The builder creates a candidate, runs the process-only packaging smoke, preserves a healthy last-known-good image, promotes only after success, and pins recurring rebuilds. The job wrapper rechecks and can restore last-known-good before running scheduled commands. |
...
Article pipeline refinement (2026-09-06): `article-editorial-review.ts` validates passage-based editorial decisions and FAQ value assessments; `article-run-report.ts` writes timing/usage reports including failures. The production inventory CLI returns resolved `public_path` values rather than requiring model-built collection URLs. Manual `articles:pipeline --revise-from` creates an explicitly authorized reviewed writing experiment from unchanged completed artifacts; exact-ID batches scope recovery to the selected ID. See `dev-docs/pipelines/articles.md` for contracts and live evidence.

Explicit completed article revisions: `articles:queue:update --status completed --result-path ...` can adopt a new verified local path for an already-completed row while preserving its slug and comparing its old result path before update. Pipeline experiments must pass their state, artifact-hash and final/browser checks before this operator handoff. It remains managed-development-only and does not publish.
