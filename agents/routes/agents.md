...
| `/api/health` | `GET` | `src/app/api/health/route.ts` | Deep operational health by default; `?scope=deploy` performs only lightweight database/build readiness for Docker and deployment gates. |
...
| `/api/roblox-profile-checker` | `GET` | `src/app/api/roblox-profile-checker/route.ts` | Username lookup aggregating public Roblox profile data with short in-memory caching; collectible results distinguish public, private, unavailable, and partial coverage and retain the upstream-build timestamp. |
