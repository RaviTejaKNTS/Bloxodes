...
When the user asks only what catalog pages can be made for a game, discovery should stay catalog-only. Resolve the game, check production `wiki_catalog_pages` and live wiki catalog routes first, check global `catalog_pages` only for overlap, research trusted online sources for durable in-game collections, and return a clean `[create]`, `[we already have a page]`, or `[skip]` catalog list. Do not expand that request into codes, articles, tools, wiki, checklist, quiz, events, gamepasses, badges, developer products, servers, raw Roblox media, or broad platform metadata.

For full game coverage, discovery should produce a compact page map, not a data-readiness report. Use full coverage only when the user asks for all page families, a complete game plan, or explicitly includes codes/wiki/articles/tools/checklist/quiz alongside catalogs. Resolve the game, check production coverage before topic ideation, decide whether a codes page should exist, list every catalog page needed for durable in-game item collections, list article topics that complete coverage without repeating other page types, identify any real tool opportunities, and mark wiki/checklist/quiz as create or already covered. If a needed page requires item data, images, or route work, recommend the page and record that work as the next action instead of marking the idea blocked. Event pages are handled by the events workflow, not by game-page discovery.
