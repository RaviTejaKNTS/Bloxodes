...
The block intentionally contains no HOT refresh. Northflank remains its sole owner for separate Roblox API/IP capacity. Hourly and daily rank jobs are VPS-owned because their long database RPCs must use the internal Supabase route rather than Cloudflare. Full-population rank jobs execute through `refresh_universe_rank_snapshots()` inside PostgreSQL so they do not page the complete sorted population through PostgREST. VPS universe collectors use `JOB_LOCK_GROUP=roblox-universe-api`, separate from item/catalog automation, because the August 2026 shared lock allowed long item jobs to silently skip 16 of 22 daily COLD runs. Critical NEW/WARM/COLD jobs wait up to 30 minutes for their universe-family lock instead of silently disappearing. WARM checks for due rows every six hours so rows that become due just after a run do not wait nearly 12 more hours; the claim gate prevents extra Roblox calls when nothing is due. Healthy COLD rows become due at 23 hours and the worker processes 4,000 rows in every hour except `00` and `12`, providing 88,000 daily slots plus one hour of headroom before the 24-hour public cutoff. The hourly `stats-current-index` job is the only scheduled full read-index rebuild.
...
# Install the exact catalog-items-discovery, catalog-items-enrichment,
# stats-items-{tier,new,hot,warm,cold,resale,index,rollup,audit} lines from:
# scripts/ops/vps-scheduled-automation.crontab
# BLOXODES_ITEM_STATS_WORKER_END
```

The tier vocabulary matches the universe pipeline: `NEW`, `HOT`, `WARM`, and `COLD`. Discovery runs daily with bounded pages/sorts and continues through all five catalog families even if one family fails. Enrichment and NEW stats run every 20 minutes, staggered ten minutes apart; HOT runs hourly, WARM every six hours, and COLD daily. Live VPS calibration uses 30-item details batches and a six-second shared request gate: a 30-row batch completed 30/30 while repeated 10-row calls hit the per-IP quota. Resale, daily rollup, the optimized hourly current-index refresh, and a strict health audit remain independently named jobs. Item, catalog, music, decal, free-item, event, and promo collectors share `JOB_LOCK_GROUP=roblox-api`; universe discovery/enrichment/refresh uses `roblox-universe-api`. This preserves per-family rate-limit coordination without allowing a ten-minute item request gate to erase universe freshness capacity. The index refresh uses durable daily rollups for 24-hour and 7-day baselines and processes at most 2,000 changed/missing index rows per call, avoiding both per-item hourly probes and long full-table rewrites.
...
```txt
/home/codex-admin/bloxodes-stats-worker/bin/run-job.sh stats-items-smoke "npm run stats:items:tier -- --apply --limit 100 && npm run enrich:catalog-items -- --max-total 10 && npm run stats:items:refresh -- --tier NEW --limit 10 --skip-index-refresh && npm run stats:items:index:refresh && npm run stats:items:audit -- --strict"
```
...
| WARM refresh | VPS worker `stats-warm-refresh` | Every 6 hours at `:32` UTC |
