...
The tier vocabulary matches the universe pipeline: `NEW`, `HOT`, `WARM`, and `COLD`. Discovery runs daily with bounded pages/sorts and continues through all five catalog families even if one family fails. Enrichment runs every 15 minutes from an atomic leased queue. Item stats refresh NEW every 15 minutes, HOT hourly, WARM every six hours, and COLD daily. Resale, daily rollup, the optimized hourly current-index refresh, and a strict health audit remain independently named jobs. Roblox-facing jobs additionally share `JOB_LOCK_GROUP=roblox-api` through `scripts/ops/vps-run-job.sh`, preventing item, universe, music, decal, free-item, event, and promo collectors from exhausting the same VPS IP allowance. The index refresh uses durable daily rollups for 24-hour and 7-day baselines and processes at most 2,000 changed rows per run, avoiding both per-item hourly probes and long full-table rewrites.
